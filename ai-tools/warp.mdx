...
Create a `WARP.md` file at the root of your documentation repository to provide Oz with project context. Warp now discovers and applies `WARP.md` and `AGENTS.md` in both local and remote (cloud agent) sessions — so the same context file covers your team whether they run Oz locally or via [cloud agents](https://docs.warp.dev/agent-platform/cloud-agents).
